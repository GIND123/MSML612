...
1,000 held-out minimal Sudoku puzzles, 20–30 clues (mean 24.4). Every puzzle was
generated by stripping cells from a solved grid and **verifying after each
removal that the solution is still unique**, so board accuracy is well defined.
Train and test solutions are disjoint **even up to digit relabelling** — an
earlier generator leaked 767/1000 and the assertion caught it before any model
trained on it.
...
| configuration | params | 1 pass | best |
|---|---|---|---|
...
Two seeds per row; the headline row is 99.60 / 99.80 and 97.4 / 97.3.

## Ablations

| ingredient | gain |
|---|---|
| recurrence vs feed-forward, same objective | **+10.80** |
| weight sharing vs 32 distinct layers | **+5.20**, at 30× fewer parameters |
| schedule matching | **+3.15** |
| deep supervision | **+1.95** |

## Recurrence extrapolates past its training depth

Trained at R = 32, evaluated at other depths (no retraining):

| R at inference | 1 | 4 | 8 | 16 | **32** | **64** | 128 |
|---|---|---|---|---|---|---|---|
...
is iterable beyond where it was fit. The honest limit is that 2× extrapolation
is reliable and 4× is not — one seed holds 99.10% at R = 128, the other collapses
to 39.70%.

## Two confounds in our own comparison, stated

**Inference depth is not matched in the headline table.** The recurrent rows use
R = 64 at inference while the 32-layer row is fixed at 32 block applications. At
**matched depth (R = 32)** the recurrent model scores **98.25%** against the
32-layer model's 94.50% — so the weight-sharing advantage is **+3.75**, not
+5.20, once depth is equalised. The larger figure should not be quoted without
this note.

**Input injection is a second difference.** The recurrent model re-adds the token
embedding at every application; the 32-layer feed-forward model does not. So
"weight sharing beats depth" is confounded with "input injection helps", and we
did not run the 32-layer-with-injection arm that would separate them. This is a
gap in the ablation, not a resolved question.
...
| Recurrent Transformer | 96.7% | RRN hardest, 17 clues | 211k |
| **ours** | **99.70%** | **ours, 20–30 clues** | **212k** |

**Different distributions; the comparison is not exact.** Our 20–30 clue range
sits inside the Recurrent Transformer's 17–34 range but excludes the hardest
17-clue instances, so 99.70% at 212k parameters should be read as *competitive
with* their 99.5% at 211k, not as beating it. The SATNet split is not used
because constraint propagation alone solves 100% of it.
...
repeat R times:
    ...
    record logits(h)                  every step is supervised
loss = Σ_r  w_r · CE(logits_r, x₀),   w_r = r/R
```

Two details carry it. **Input injection** keeps the clues alive through thirty
applications of the same block. **Deep supervision** forces each application to
be a valid one-step refinement rather than letting the stack learn one entangled
R-step function, and it is what makes inference at R = 64 work after training at
R = 32.
